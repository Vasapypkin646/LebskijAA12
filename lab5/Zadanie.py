...
try:
    ...
    print(f"Ошибка импорта паттернов: {e}")
    print("Убедитесь, что структура проекта корректна")

# Дополнительные методы для классов с использованием паттернов

class EnhancedDepartment(Department):
    """Расширенный класс Department с использованием паттернов"""
    
    def __init__(self, name: str):
        super().__init__(name)
        self._employee_repository = EmployeeRepository()
        self._bonus_context = BonusContext(PerformanceBonusStrategy())
    
    def add_employee_with_patterns(self, employee: AbstractEmployee):
        """Добавляет сотрудника с использованием паттернов"""
        # Используем Repository для сохранения в БД
        success = self._employee_repository.add(employee)
        if success:
            self.add_employee(employee)
            print(f"Сотрудник {employee.name} добавлен в отдел {self.name} и сохранен в БД")
        return success
    
    def calculate_bonus_for_all(self, strategy_type: str = "performance", **kwargs):
        """Рассчитывает бонусы для всех сотрудников отдела с использованием Strategy"""
        
        # Выбираем стратегию
        if strategy_type == "performance":
            self._bonus_context.strategy = PerformanceBonusStrategy()
        elif strategy_type == "seniority":
            self._bonus_context.strategy = SeniorityBonusStrategy()
        elif strategy_type == "project":
            self._bonus_context.strategy = ProjectBonusStrategy()
        else:
            print(f"Неизвестный тип стратегии: {strategy_type}")
            return {}
        
        # Рассчитываем бонусы
        bonuses = {}
        for employee in self.spis:
            bonus = self._bonus_context.calculate_bonus(employee, **kwargs)
            bonuses[employee.id] = {
                'name': employee.name,
                'bonus': bonus,
                'total_salary': employee.calculate_salary() + bonus
            }
        
        return bonuses

class EnhancedCompany(Company):
    """Расширенный класс Company с использованием паттернов"""
    
    def __init__(self, name: str, departments: list[Department] = [], projects: list[Project] = []):
        super().__init__(name, departments, projects)
        self._db = DatabaseConnection()
        self._salary_adapter = SalaryCalculatorAdapter()
        self._employee_repository = EmployeeRepository()
    
    def calculate_external_salaries(self):
        """Рассчитывает зарплаты всех сотрудников через внешнюю систему (Adapter)"""
        all_employees = self.get_all_employees()
        
        results = {}
        for employee in all_employees:
            external_salary = self._salary_adapter.calculate_salary(employee)
            ...
            results[employee.id] = {
                ...
                'percentage_diff': ((external_salary - internal_salary) / internal_salary * 100) if internal_salary > 0 else 0
            }
        
        return results
    
    def save_all_to_database(self):
        """Сохраняет всех сотрудников компании в БД через Repository"""
        all_employees = self.get_all_employees()
        
        saved_count = 0
        for employee in all_employees:
            if self._employee_repository.add(employee):
                saved_count += 1
        
        print(f"Сохранено {saved_count} из {len(all_employees)} сотрудников в БД")
        return saved_count
    
    def generate_advanced_report(self):
        """Генерирует расширенный отчет с использованием всех паттернов"""
        print(f"\n{'='*60}")
        print(f"РАСШИРЕННЫЙ ОТЧЕТ КОМПАНИИ: {self.name}")
        print(f"{'='*60}")
        
        # 1. Информация о паттерне Singleton
        print(f"\n1. ПАТТЕРН SINGLETON:")
        db = DatabaseConnection()
        print(f"   Используется единое подключение к БД: {db.database_file}")
        
        # 2. Расчет зарплат через Adapter
        print(f"\n2. ПАТТЕРН ADAPTER (внешний расчет зарплат):")
        external_salaries = self.calculate_external_salaries()
        for emp_id, data in list(external_salaries.items())[:3]:  # Показываем первые 3
            print(f"   {data['name']}:")
            print(f"     Внутренняя зарплата: {data['internal_salary']:.2f}")
            print(f"     Внешняя зарплата: {data['external_salary']:.2f}")
            print(f"     Разница: {data['difference']:+.2f} ({data['percentage_diff']:+.1f}%)")
        
        # 3. Использование Repository
        print(f"\n3. ПАТТЕРН REPOSITORY (работа с БД):")
        total_in_db = len(self._employee_repository.get_all())
        total_in_company = sum(len(dept) for dept in self._Company__departments)
        print(f"   Сотрудников в компании: {total_in_company}")
        print(f"   Сотрудников в БД: {total_in_db}")
        
        # 4. Использование Strategy для расчета бонусов
        print(f"\n4. ПАТТЕРН STRATEGY (расчет бонусов):")
        for department in self._Company__departments:
            if hasattr(department, 'calculate_bonus_for_all'):
                bonuses = department.calculate_bonus_for_all("performance", performance_score=1.2)
                if bonuses:
                    total_bonus = sum(data['bonus'] for data in bonuses.values())
                    print(f"   Отдел {department.name}: общая сумма бонусов = {total_bonus:.2f}")


# ДЕМОНСТРАЦИЯ РАБОТЫ ПАТТЕРНОВ

def demonstrate_patterns_integration():
    """Демонстрация интеграции паттернов в существующую систему"""
    
    print("\n" + "="*70)
    print("ДЕМОНСТРАЦИЯ ИНТЕГРАЦИИ ПАТТЕРНОВ В СИСТЕМУ УЧЕТА СОТРУДНИКОВ")
    print("="*70)
    
    # Создаем тестовые данные из вашего кода
    tom2 = Employee(1, "Tom2", "2", 30001)
    Bread = Manager(3, "Bread", "4", 30000, 500)
    Stiles = Developer(4, "Stiles", "5", 30000, ["C++","C","Python"], "middle")
    Brenda = Saleperson(5, "Brenda", "4", 30000, 0.1, 5000)
    ...
    print(f"   Файл БД: {db1.database_file}")
    
    # 2. Демонстрация Adapter
    print("\n2. ДЕМОНСТРАЦИЯ ADAPTER:")
    adapter = SalaryCalculatorAdapter()
    
    print("   Расчет зарплат через внешнюю систему:")
    employees = [tom2, Bread, Stiles, Brenda]
    for emp in employees:
        internal = emp.calculate_salary()
        external = adapter.calculate_salary(emp)
        diff = external - internal
        print(f"   {emp.name:10} ({emp.__class__.__name__:12}):")
        print(f"     Внутренняя: {internal:8.2f}")
        print(f"     Внешняя:    {external:8.2f}")
        print(f"     Разница:    {diff:8.2f}")
    
    # 3. Демонстрация Strategy
    print("\n3. ДЕМОНСТРАЦИЯ STRATEGY:")
    
    # Стратегия производительности
    context = BonusContext(PerformanceBonusStrategy())
    print(f"   Стратегия производительности для менеджера:")
    bonus_perf = context.calculate_bonus(Bread, performance_score=1.3)
    print(f"   Бонус Bread: {bonus_perf:.2f}")
    
    # Смена стратегии на стажевую
    context.strategy = SeniorityBonusStrategy()
    bonus_sen = context.calculate_bonus(Stiles, seniority_years=3)
    print(f"   Стратегия стажа для разработчика (3 года):")
    print(f"   Бонус Stiles: {bonus_sen:.2f}")
    ...
    for emp in employees:
        if repo.add(emp):
            print(f"   Сохранен: {emp.name}")
    
    # Получаем всех из БД
    all_from_db = repo.get_all()
    print(f"   Всего в БД: {len(all_from_db)} сотрудников")
    
    # Поиск по отделу
    dept4 = repo.find_by_department("4")
    print(f"   В отделе '4': {len(dept4)} сотрудников")
...
# ОСНОВНАЯ ПРОГРАММА С ИНТЕГРИРОВАННЫМИ ПАТТЕРНАМИ

if __name__ == "__main__":
    # Запускаем демонстрацию паттернов
    demonstrate_patterns_integration()
    
    # Дополнительно: создаем расширенную компанию
    print("\n\nСОЗДАНИЕ РАСШИРЕННОЙ КОМПАНИИ С ПАТТЕРНАМИ:")
    print("-"*50)
    
    # Создаем расширенную компанию
    enhanced_company = EnhancedCompany("TechCorp с паттернами")
    
    # Создаем расширенный отдел
    dev_department = EnhancedDepartment("Разработка")
    
    # Добавляем сотрудников с использованием паттернов
    developer1 = Developer(1001, "Алексей Программистов", "Разработка", 80000, 
                          ["Python", "JavaScript", "Docker"], "senior")
    developer2 = Developer(1002, "Мария Кодова", "Разработка", 60000,
                          ["Java", "Spring", "SQL"], "middle")
    
    dev_department.add_employee_with_patterns(developer1)
    dev_department.add_employee_with_patterns(developer2)
    
    enhanced_company.add_department(dev_department)
